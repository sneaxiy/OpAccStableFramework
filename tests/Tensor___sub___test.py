...
class Tensor__sub__Test:
    ...
    def run_paddle(self, paddle):
        x = self.inputs["x"]
        y = self.inputs["y"]
        y = paddle.Tensor.__sub__(x, y)
        return y

    def run_torch(self, torch):
        x = self.inputs["x"]
        y = self.inputs["y"]
        y = torch.Tensor.__sub__(x, y)
        return y
...
if __name__ == "__main__":
    op_acc_stable_run(Tensor__sub__Test(x_shape = [1, 8192], x_dtype = "float32", y_shape = [1, 8192], y_dtype = "float32"))
    op_acc_stable_run(Tensor__sub__Test(x_shape = [1, 1024, 254208], x_dtype = "float32", y_shape = [1, 1024, 1], y_dtype = "float32"))
    op_acc_stable_run(Tensor__sub__Test(x_shape = [1, 1024, 1], x_dtype = "float32", y_shape = [1, 1024, 254208], y_dtype = "float32"))
