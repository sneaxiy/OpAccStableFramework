...
class MaxTest:
    ...
    def run_paddle(self, paddle):
        x = self.inputs["x"]
        y = paddle.max(x, self.axis, self.keepdim)
        return y

    def run_torch(self, torch):
        x = self.inputs["x"]
        y = torch.max(x, self.axis, self.keepdim)
        return y
...
if __name__ == "__main__":
    op_acc_stable_run(MaxTest(x_shape = [1, 1024, 254208], dtype='float32', axis=-1, keepdim=True))
