    ...
    with open(API_DIFF_SPEC_FN) as f:
        for line in f.readlines():
            ...
            if hasattr(api_obj, "__doc__") and api_obj.__doc__:
                sample_code_filenames = sampcd_extract_to_file(api_obj.__doc__, api)
...
def get_api_md5(path):
    ...
    patArgSpec = re.compile(r"^(paddle[^,]+)\s+\(ArgSpec.*document\W*([0-9a-z]{32})")
...
if __name__ == "__main__":
    ...
    if args.mode == "gpu":
        ...
        logger.error("Unrecognized argument:%s, 'cpu' or 'gpu' is desired.", args.mode)
        sys.exit("Invalid arguments")
    RUN_ON_DEVICE = args.mode
    get_test_capacity()
    logger.info("API check -- Example Code")
    logger.info("sample_test running under python %s", platform.python_version())
    ...
    if len(whl_error) != 0:
        ...
        if len(timeovered_test):
            logger.info("%d sample codes ran time over 10s", len(timeovered_test))
            if args.debug:
                for k, v in timeovered_test.items():
                    logger.info(f"{k} - {v}s")
        if len(SUMMARY_INFO["success"]):
            logger.info("%d sample codes ran success", len(SUMMARY_INFO["success"]))
        ...
        if len(SUMMARY_INFO["skiptest"]):
            logger.info("%d sample codes skipped", len(SUMMARY_INFO["skiptest"]))
            if args.debug:
                logger.info("\n".join(SUMMARY_INFO["skiptest"]))
        if len(SUMMARY_INFO["nocodes"]):
            logger.info("%d apis don't have sample codes", len(SUMMARY_INFO["nocodes"]))
            if args.debug:
                logger.info("\n".join(SUMMARY_INFO["nocodes"]))
        if len(SUMMARY_INFO["failed"]):
            logger.info("%d sample codes ran failed", len(SUMMARY_INFO["failed"]))
