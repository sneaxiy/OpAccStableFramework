...
parser.add_argument("--git_dir", type=str, default="", help="git repo root directory.")
parser.add_argument("--build_dir", type=str, default="", help="build directory.")
parser.add_argument(
    "--good_commit",
    type=str,
    default="",
...
parser.add_argument("--build_parallel", type=int, default=32, help="make parallelism.")
...
while True:
    ...
    subprocess.check_output(
        ["git checkout %s && git clean -fd && git checkout ." % args.bisect_branch],
        shell=True,
    )
    ...
    cmd = "rm -rf * && " "cmake -DWITH_TESTING=ON %s >> %s && make -j%s >> %s" % (
        args.git_dir,
        args.log_file,
        args.build_parallel,
        args.log_file,
