...
def check_public_api():
    ...
    for module, allapi in alldict.items():
        for member_name in allapi:
            ...
            member_dict[cur_name] = "({}, ('document', '{}'))".format(cur_name, doc_md5)
...
def parse_args():
    ...
    parser.add_argument("module", type=str, help="module", default="paddle")  # not used
    parser.add_argument(
        "--skipped",
        dest="skipped",
        type=str,
    ...
    if args.method == "from_modulelist":
        ...
        for api_name in all_api_names_sorted:
            ...
            print(
                "{} ({}, ('document', '{}'))".format(
                    api_name,
                    api_info["signature"] if "signature" in api_info else "ArgSpec()",
