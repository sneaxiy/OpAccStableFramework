...
def append_fluid_kernels():
    ...
    for op in op_white_list:
        append_str = (
            append_str + 'file(APPEND ${pybind_file} "USE_OP__(%s);\\n")\n' % op
        )
    ...
    for op_file in all_op:
        ...
        for op in op_white_list:
            patterns = {
                ...
                "REGISTER_OP_CPU_KERNEL": r"REGISTER_OP_CPU_KERNEL\(\s*%s\s*," % op,
                "REGISTER_OP_CUDA_KERNEL": r"REGISTER_OP_CUDA_KERNEL\(\s*%s\s*," % op,
            }
