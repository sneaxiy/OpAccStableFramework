...
from CspFileReader import (DCGMINFO_TRACE_NUM, FILEORGANIZEFORM_BYRANK,
                           NETINFO_TRACE_NUM, PIPELINEINFO_TRACE_NUM,
                           FileReader, getLogger)
...
class profileFileReader(FileReader):
        ...
        for k, profile_pb in profile_dict.items():
                ...
                if event.type == profiler_pb2.Event.CPU:
                    if (k, event.device_id, "CPU") not in devices:
                        ...
                        if event.device_id == -1:
                            ...
                            chrome_trace.emit_pid(
                                "%02d_%s:cpu:block:%d" % (lineNum, k, event.device_id),
                                pid,
                            )
                            lineNum = lineNum + 1
                    ...
                    if (k, event.device_id, "GPUKernel") not in devices:
                        if gpuId == event.device_id:
                            ...
                            chrome_trace.emit_pid(
                                "%02d_%s:gpu:%d" % (lineNum, k, event.device_id),
                                pid,
                            )
                            lineNum = lineNum + 1
            ...
            for mevent in profile_pb.mem_events:
                ...
                if (k, 0, "CUDAPinnedPlace") not in mem_devices:
                    ...
                    chrome_trace.emit_pid(
                        "%02d_memory usage on %s:cudapinnedplace:%d" % (lineNum, k, 0),
                        pid,
                    )
                    lineNum = lineNum + 1
            i = i + 1
    ...
    def _getOPTraceInfoByGpuId(self, groupId, gpuId):
        ...
        for file in fileFist:
            ...
            if (
                localRank == gpuId
                and (rankId / self._gpuPerTrainer) % self._groupSize < self._displaySize
            ):
